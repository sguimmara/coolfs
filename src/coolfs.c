...
int cool_read(const char *path, char *buf, size_t size, off_t offset,
  ...
  log_debug("cool_read: %s (bufsize: %zo)", path, size);

  cool_inode *inode = cool_find_inode(path);

  if (inode != NULL) {
    off_t size = inode->st->st_size;
    log_trace("cool_read: copying content of %lo bytes", size);
...
int cool_getattr(const char *path, struct stat *st) {
  ...
  if (inode != NULL) {
    ...
    log_debug("cool_getattr: %s -> success (stat: %x)", path, inode->st);
    return 0;
  }

  log_debug("cool_getattr: %s -> ENOENT", path);
  return -ENOENT;
}
...
int cool_readdir(const char *path, void *buf, fuse_fill_dir_t filler,
  ...
  log_trace("cool_readdir: %s", path);

  cool_inode *inode = cool_find_inode(path);

  if (inode == NULL) {
    log_trace("cool_readdir: %s -> ENOENT", path);
    return -ENOENT;
  }

  log_trace("cool_readdir: %s (%lo children)", inode->name, inode->children->count);
